...
public class SalaryUploadServiceImpl implements FileUploadService {
	...
	public String upload(MultipartFile file, Object... objects) throws BohaiException {
		...
		try {
		    ...
		    String unemploymentPersonalPercent=Double.toString(salarySheet.getRow(5).getCell(8).getNumericCellValue());
			...
			String unemploymentCompanyPercent=Double.toString(salarySheet.getRow(5).getCell(13).getNumericCellValue());
			...
			for(int i = 6;i<salarySheet.getLastRowNum();i++){
				if (salarySheet.getRow(i)!=null) {
					...
					if (StringUtils.isNotEmpty(salarySheet.getRow(i).getCell(2).getStringCellValue())) {
						   ...
						   /* HSSFRow row = salarySheet.getRow(i);
							HSSFCell cell = salarySheet.getRow(i).getCell(2);
